...
contract NewLeaseContract {
    ...
    mapping(string => bool) public leasedCIDs;
    string[] public cids;
    ...
    function getAllUploads() external view returns (string[] memory allCids, address[] memory allLeasers) {
        ...
        for (uint i = 0; i < count; i++) {
            if (!leasedCIDs[cids[i]]) { // Check if CID is not leased
                allCids[i] = cids[i];
    ...
    function getLeaseDetails(string memory _cid) external view returns (address leaser, uint duration, uint payment) {
    require(cidExists[_cid], "CID does not exist");
    Lease memory lease = leases[cidToLeaser[_cid]];
    return (lease.leaser, lease.duration, lease.payment);
    }
    function isCIDLeased(string memory _cid) external view returns (bool) {
        return leasedCIDs[_cid];
    }
